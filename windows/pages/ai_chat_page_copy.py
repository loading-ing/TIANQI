...
class AIChatPage(QWidget):
    ...
    def create_settings_group(self):
        ...
        if self.model_config:
            model_names=[key for key,value in self.model_config["session"]]
        else:
        ...
        self.explain_combo.setCurrentIndex(3)
    ...
    def generate_causal_response(self, user_message):
        ...
        responses = {
            "What is the causal effect of X on Y?": 
                f"Determining the causal effect of X on Y requires consideration of potential confounders. " 
                f"Using {model}, we can employ techniques like propensity score matching or instrumental variables. "
                f"The estimated effect depends on the study design and data quality.",
                
            "How to identify confounding variables?":
                f"Confounding variables are those that affect both the treatment (X) and outcome (Y). " 
                f"With {model}, we can use causal diagrams (DAGs) to identify potential confounders. "
                f"Common approaches include backdoor criterion and covariate adjustment.",
                
            "Explain Simpson's paradox with an example":
                "Simpson's paradox occurs when a trend appears in different groups but disappears or reverses when groups are combined. "
                "Example: A drug may appear harmful when looking at aggregated data but beneficial when examining subgroups by age. "
                "This highlights the importance of considering confounding variables in causal analysis.",
                
            "What is the difference between correlation and causation?":
                "Correlation indicates a statistical association between variables, while causation implies that one variable directly influences another. "
                f"Using {model}, we can distinguish them through controlled experiments or causal inference methods. "
                "Remember: correlation does not imply causation!",
                
            "How to design a causal experiment?":
                "Key elements of causal experiments:\n"
                "1. Random assignment to treatment/control groups\n"
                "2. Control of confounding variables\n"
                "3. Adequate sample size\n"
                "4. Clear definition of treatment and outcome\n"
                f"With {model}, you can also analyze observational data when experiments are impractical.",
                
            "Explain instrumental variables":
                "Instrumental variables (IV) are used when there's unmeasured confounding between X and Y. "
                "An IV must:\n"
                "1. Be correlated with X (relevance)\n"
                "2. Affect Y only through X (exclusion)\n"
                "3. Not share common causes with Y (exchangeability)\n"
                f"Using {model}, IV analysis can provide unbiased causal estimates in such scenarios."
        }
